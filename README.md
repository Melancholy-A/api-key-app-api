...
- 聊天备份：历史面板可将聊天和全部回答分支导出为 JSON，也可从 JSON 恢复；恢复前会显示聊天数和消息数，同 ID 聊天覆盖、其他本地聊天保留。备份不包含 API key、接口地址、应用设置、生成图片和 Office 文件。
...
打开历史面板，点击“备份”后选择保存位置；恢复时选择之前导出的 JSON 文件，确认对话框会列出聊天数和消息数。恢复采用事务写入：同 ID 会话被备份内容覆盖，未出现在备份中的本地会话保留；JSON 格式错误、版本不支持或文件过大时不会修改现有历史。

备份只保存聊天正文、回答分支元数据、标题和时间等历史信息，不保存 API key、Base URL、模型与其他应用设置，也不会打包生成图片或 Office 文件。恢复后这些本地文件不会被重新生成。
