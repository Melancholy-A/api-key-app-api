...
- 上下文聊天：本地 SQLite 保存历史会话，打开历史后可以继续追问，旧 JSON 历史会自动迁移；新增消息、分支切换和截断只写入变化位置，最多保留 50 个会话。
...
- 图片生成：点击生图按钮时走 `/v1/images/generations`；普通发送里出现“生成图片/画示意图/设计海报”等表达时，会优先用文字和 `text` 代码块给出字符示意图，避免误触生图接口导致乱码或卡顿。
- 类 ChatGPT 移动端体验：消息复制、朗读、分享、重新生成、停止请求、编辑用户消息、每条消息补充修改要求、代码块复制和滚动到底部按钮。
- 回答分支：重答或修改要求会生成同一节点下的新回答分支，旧分支及其后续对话会保留，可通过 `1/2` 分支控件切回。
- 聊天备份：历史面板可将聊天和全部回答分支导出为 JSON，也可从 JSON 恢复；恢复前会显示聊天数和消息数，同 ID 聊天覆盖、其他本地聊天保留。达到 50 个会话上限时，可能挤掉本地聊天的导入会被拒绝。备份不包含 API key、接口地址、应用设置、生成图片和 Office 文件。
...
图片生成按钮走 `/v1/images/generations`。如果第三方接口对模型名称有限制，可以在设置中把生图模型改为它支持的模型，例如 `image-2`。

生成的图片会保存到 App 本地文件，并进入本地图库；历史聊天中也能继续查看。

普通聊天里如果只是让它“画一个示意图”，App 会要求模型输出文字/ASCII 示意图；需要真实图片时请使用底部生图按钮。
...
打开历史面板，点击“备份”后选择保存位置；恢复时选择之前导出的 JSON 文件，确认对话框会列出聊天数和消息数。备份与恢复的文件读写、JSON 编解码和 SQLite 事务都在后台执行。恢复采用事务写入：同 ID 会话被备份内容覆盖，未出现在备份中的本地会话保留；如果新增会话会超过 50 个上限，恢复会在写入前拒绝，避免静默删除本地聊天。JSON 格式错误、结构不完整、版本不支持或文件过大时不会修改现有历史。

备份只保存聊天正文、回答分支元数据、标题和时间等历史信息，并在导出时限制为最多 50 个聊天、100,000 条消息和 16 MiB。它不保存 API key、Base URL、模型与其他应用设置，也不会打包本地或远程生成图片、Office 文件。恢复后这些本地文件不会被重新生成。
